...
def show_update_detail():
    ...
    if request.method=='POST':
        ...
        qry = "Select * from User_data where User_ID = %s" %(request.form['User_ID'])
        ...
        qry_upd = "Select * from User_data where User_ID = %s" %(request.form['User_ID'])
        ...
        if "delete" in request.form:
            if not_found:
                ...
                qry2 = "DELETE FROM User_data where User_ID = %s" %(request.form['User_ID'])
...
def update_details():
    ...
    try:
        ...
        print("update error")
    mydb.commit()
    qry2 = "select * from User_data where User_ID = %s" %(request.form['User_ID'])
    mycursor.execute(qry2)
    res = mycursor.fetchone()
    qry = "Select * from User_data where User_ID = %s" %(request.form['User_ID'])
    qry1 = "Select * from User_phone_no where User_ID = %s" %(request.form['User_ID'])
...
def del_hostel():
    if not session.get('login'):
        return redirect( url_for('home') )
    qry = "delete from User_data where User_ID="+str(request.form['User_ID'])
    print(qry)
    try:
        mycursor.execute(qry)
    except:
